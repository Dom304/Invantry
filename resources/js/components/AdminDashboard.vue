<template>
    <div class="left-window-admin-dashboard">
        ...
        <button class="window-btn" :class="{ selected: currentwindow === 'user' }" @click="currentwindow = 'user'">
            Users
        </button>
    ...
    <div class="right-window-admin-dashboard">
        <user-table v-if="currentwindow === 'user'" :users="users" :logged-in-user-id="loggedInUserId" ref="userTable"
            :is-busy="isFetchingUsers" @refresh-users="fetchUsers"></user-table>

        <store-table v-else-if="currentwindow === 'store'" :stores="stores" :is-busy="isFetchingStore"
            @refresh-stores="fetchStores"></store-table>

        <request-table v-else-if="currentwindow === 'request'" :manager-requests="managerRequests"
            :is-busy="isFetchingRequests" @refresh-requests="fetchRequests"></request-table>
    </div>
    <Toast />
</template>
    ...
    methods: {
        async fetchUsers() {
            ...
            try {
                ...
                this.$toast.add({ severity: 'success', summary: 'Success', detail: 'Action successful', life: 60000 });
